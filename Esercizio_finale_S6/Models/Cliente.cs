...
namespace Esercizio_finale_S6.Models
{
    public class Cliente
    {
        ...
        public static List<SelectListItem> GetCodiciFiscali()
        {
            List<SelectListItem> codiciFiscali = new List<SelectListItem>();

            using (SqlConnection sqlConnection = new SqlConnection(GetConnectionString()))
            {
                sqlConnection.Open();

                string query = "SELECT CodiceFiscale FROM Clienti"; 
                ...
                using (SqlCommand cmd = new SqlCommand(query, sqlConnection))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            string codiceFiscale = reader["CodiceFiscale"].ToString();
                            codiciFiscali.Add(new SelectListItem
                            {
                                Text = codiceFiscale,
                                Value = codiceFiscale
                            });
